...
for(const [file,html] of pages){
 ...
 const selector = html.match(/<select\b[^>]*class="theme-select"[^>]*>([\s\S]*?)<\/select>/)?.[1];
 if (!selector || !['light','dark','river'].every(mode=>selector.includes(`value="${mode}"`))) errors.push(`${file}: three-mode selector missing`);
...
for(const system of [false,true])for(const stored of [null,'light','dark','river','invalid','throws']){
 ...
 const expected=['light','dark','river'].includes(stored)?stored:system?'dark':'light';
 if(document.documentElement.dataset.theme!==expected)errors.push('Theme initialization failed');
}
if(errors.length){console.error(errors.join('\n'));process.exit(1)}
console.log(`Verified ${files.length} routes, ${count} local links/assets, anchor targets, image alternatives, three-mode selectors, and 12 theme initialization cases.`);
